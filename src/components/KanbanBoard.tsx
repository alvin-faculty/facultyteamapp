...
function Column({
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(
        'flex h-full w-72 shrink-0 flex-col rounded-lg bg-card p-2',
        isOver && 'bg-muted/50',
      )}
    >
      <div className='flex items-center justify-between px-1 pb-2'>
        <div className='flex items-center gap-2'>
          ...
          />
          <span className='heading-4'>{section.name}</span>
        </div>
...
export function KanbanBoard({
  ...
  const assigneesByTask = taskAssignees.reduce<Record<string, Profile[]>>(
    (acc, ta) => {
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const position = localTasks.filter(
      (t) => t.section_id === newSectionId,
    ).length;

    setLocalTasks((prev) =>
      prev.map((t) =>
        t.id === task.id ? { ...t, section_id: newSectionId, position } : t,
      ),
    );
  ...
  const selectedAssignees = selectedTask
    ? (assigneesByTask[selectedTask.id] ?? [])
    : [];
  ...
  return (
    <div className='col-span-12 space-y-3'>
      <div className='flex items-baseline justify-between'>
        <h2 className='heading-4 uppercase'>Workflow</h2>
        {topLevelTasks.length > 0 && (
