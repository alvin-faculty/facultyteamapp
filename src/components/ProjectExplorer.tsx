...
import type { Client, Profile, Project, ProjectStatus } from "@/lib/supabase/types";
...
function usagePct(s: ProjectSummary): number {
  const hoursPct = s.project.budget_hours ? (s.usedMinutes / 60 / s.project.budget_hours) * 100 : null;
  const amountPct = s.project.budget_amount ? (s.usedAmount / s.project.budget_amount) * 100 : null;
...
function BoardColumn({
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn("w-72 shrink-0 space-y-3 rounded-lg p-1", isOver && "bg-muted/50")}
    >
      <div className="flex items-center gap-2 px-1">
        <span className={cn("size-2 rounded-full", sectionColorClass(colorIndex))} />
        <span className="text-[11px] font-semibold tracking-[0.05em] uppercase">{label}</span>
...
export function ProjectExplorer({
  ...
  if (search.trim()) {
    ...
    filtered = filtered.filter(
      (s) => s.project.name.toLowerCase().includes(q) || s.client?.name.toLowerCase().includes(q),
    );
  }
  if (clientFilter !== "all") filtered = filtered.filter((s) => s.client?.id === clientFilter);
  ...
  const columnsToShow =
    statusFilter === "all" ? STATUSES.filter((s) => s.value !== "archived") : STATUSES.filter((s) => s.value === statusFilter);
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    setLocalSummaries((prev) =>
      prev.map((s) =>
        s.project.id === summary.project.id ? { ...s, project: { ...s.project, status: newStatus } } : s,
      ),
    );
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Select
            ...
            items={{ all: "All clients", ...Object.fromEntries(clients.map((c) => [c.id, c.name])) }}
          >
      ...
      <Tabs defaultValue="list">
        ...
        <TabsContent value="list" className="space-y-6 pt-4">
          {STATUSES.map((status, statusIndex) => {
            const items = sorted.filter((s) => s.project.status === status.value);
            if (items.length === 0) return null;
            return (
              <div key={status.value} className="space-y-3">
                <div className="flex items-center gap-2">
                  <span className={cn("size-2 rounded-full", sectionColorClass(statusIndex))} />
                ...
                <div className="grid grid-cols-12 gap-6">
                  {items.map((s) => (
                    <div key={s.project.id} className="col-span-12 sm:col-span-6 lg:col-span-4">
                      <ProjectChip
                        project={s.project}
                        client={s.client}
