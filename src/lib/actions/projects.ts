...
export async function updateProjectBudgetHours(
  projectId: string,
  hours: number | null,
) {
  await requireAdmin();
...
export async function uploadProposalScope(
  projectId: string,
  formData: FormData,
) {
  const supabase = await createSupabaseServerClient();
...
export async function deleteProposalScope(projectId: string, url: string) {
  const supabase = await createSupabaseServerClient();

  // Extract the storage path from the public URL, e.g.
  // https://xxx.supabase.co/storage/v1/object/public/proposal-scopes/<projectId>/<file>
  const marker = '/proposal-scopes/';
  ...
  const { error } = await supabase
    .from('projects')
    .update({ proposal_scope_url: null })
    .eq('id', projectId);
  if (error) throw new Error(error.message);
